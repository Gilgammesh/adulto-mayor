...
const Index = () => {
  ...
  const [createPaciente] = useMutation(CREATE_PACIENTE, {
    onCompleted({ createPaciente }) {
      if (createPaciente) {
        const { estado, msg } = createPaciente;
        if (!estado) {
          Swal.fire({
            title: "Error",
            text: msg,
  ...
  const handleChangeInputName = (evt) => {
    ...
    setState({
      ...state,
      nombres: `${apellidoPaterno.trim()} ${apellidoMaterno.trim()} ${value
        .toUpperCase()
        .trim()}`,
    });
    var elements = document.getElementsByClassName("leaflet-control-geocoder");
  ...
  const handleChangeInputPattern = (evt) => {
    ...
    setState({
      ...
      nombres: `${value
        .toUpperCase()
        .trim()} ${apellidoMaterno.trim()} ${nombres.trim()}`,
    });
    var elements = document.getElementsByClassName("leaflet-control-geocoder");
    while (elements.length > 0) {
      elements[0].parentNode.removeChild(elements[0]);
  ...
  const handleChangeInputMattern = (evt) => {
    ...
    setState({
      ...state,
      nombres: `${apellidoPaterno.trim()} ${value
        .toUpperCase()
        .trim()} ${nombres.trim()}`,
    });
    var elements = document.getElementsByClassName("leaflet-control-geocoder");
                ...
                <>
                  <div className={classes.containerHeader}>
                    <div className={classes.containerHeaderTit}>
                      <PersonAddIcon className={classes.icontitle} />
